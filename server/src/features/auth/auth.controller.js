...
export async function verifyEmail(req, res, next) {
  try {
    ...
    if (!token) return next({ statusCode: 400, message: 'الرمز مفقود', code: 'MISSING_TOKEN' });
...
export async function refresh(req, res, next) {
  try {
    ...
    if (!rawToken) return next({ statusCode: 401, message: 'الجلسة غير موجودة', code: 'NO_SESSION' });
